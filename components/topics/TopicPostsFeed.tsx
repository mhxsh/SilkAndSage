...
export default function TopicPostsFeed({ topicId, lang, dict }: TopicPostsFeedProps) {
    ...
    const fetchPosts = async () => {
        setLoading(true)
        try {
            const res = await fetch(`/api/topics/${topicId}/posts?limit=20&offset=${offset}`)
            if (res.ok) {
        ...
        return (
            <div className="bg-white rounded-lg shadow-md p-12 text-center">
                <p className="text-gray-600 mb-4">
                    {lang === 'zh' ? '这个话题还没有内容，快来发布第一条吧！' : 'No posts yet for this topic. Be the first to share!'}
                </p>
                <Link
                    href={`/${lang}/ugc/create?topic=${topicId}`}
